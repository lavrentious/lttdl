...
function deleteExpiredFiles(): void {
  ...
  const rows = database
    .query("SELECT id, file_path FROM shared_files WHERE expire_at <= datetime('now')")
  ...
  for (const row of rows) {
    try {
      ...
      logger.warn(`file-share: failed to delete ${row.file_path}: ${String(err)}`);
    }
  }
...
export function initFileShare(): void {
  ...
  if (!baseUrl) {
    throw new Error("FILE_SHARE_ENABLED is true but FILE_SHARE_BASE_URL is not set");
  }
...
export async function shareFile(sourcePath: string): Promise<string | null> {
  ...
  try {
    ...
    const expireAt = new Date(Date.now() + config.get("FILE_SHARE_TTL_S") * 1000)
      .toISOString()
      .replace("T", " ")
      .slice(0, 19);
    db.query("INSERT INTO shared_files (id, file_path, expire_at) VALUES (?, ?, ?)").run(
      id,
      destPath,
      expireAt,
    );
